...
namespace Core {
	...
	void X11::Init(const ax::Size& frame_size)
	{
		...
		_win = XCreateWindow(_display, _rootWindow, 0, 0, s_size.x, s_size.y, 0, _visualInfo->depth,
			InputOutput, _visualInfo->visual, CWColormap | CWEventMask, &swa);
	...
	{
		return ax::Size(XDisplayWidth(_display, _screen), XDisplayHeight(_display, _screen));
	}

	ax::Rect X11::GetScreenRect()
	{
		return ax::Rect(0, 0, XDisplayWidth(_display, _screen), XDisplayHeight(_display, _screen));
	}
	...
	XKeyEvent createKeyEvent(
		Display* display, x11::Window& win, x11::Window& winRoot, bool press, int keycode, int modifiers)
	{
		XKeyEvent event;
	...
	void ax::Core::X11::MainLoop()
	{
		...
		_drawing_timer = new ax::Event::Timer(
			app.GetEventManager(), ax::Event::Function([&](ax::Event::Msg* msg) { _refresh = true; }));
		...
		while (loop_on) {
			...
			if (XPending(_display)) {
				...
				case MotionNotify:
					manager->OnMouseMotion(ax::Point(e.xmotion.x, e.xmotion.y));
					break;
				case ButtonPress:
	...
	x11::Window ax::Core::X11::CreateWindow(
		Display* dpy, const ax::Rect& rect, x11::Window& root_win, Colormap& cmap, XVisualInfo* v_info)
	{
		...
		return XCreateWindow(dpy,
			root_win, // Parent.
			rect.position.x, rect.position.y, rect.size.x, rect.size.y, 0, v_info->depth, InputOutput,
			v_info->visual, CWColormap | CWOverrideRedirect, &attributes);
	}
}
}
