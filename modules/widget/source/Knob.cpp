...
std::vector<std::string> Knob::Info::GetParamNameList() const
{
	return std::vector<std::string>{ "knob_size", "n_knob", "bgColorNormal", "bgColorHover",
		"bgColorClicked", "img_path", "selected_img_path" };
}

std::string Knob::Info::GetAttributeValue(const std::string& name)
{
	if (name == "knob_size") {
		return std::to_string(knob_size.w) + ", " + std::to_string(knob_size.h);
	}
	else if (name == "n_knob") {
...
void Knob::Info::SetAttribute(const std::pair<std::string, std::string>& attribute)
{
	if (attribute.first == "knob_size") {
		knob_size = ax::Size(attribute.second);
	}
...
ax::Xml::Node Knob::Component::Save(Xml& xml, Xml::Node& node)
{
	...
	info_node.AddAttribute(
		"knob_size", std::to_string(info->knob_size.w) + ", " + std::to_string(info->knob_size.h));
...
std::vector<std::pair<std::string, std::string>> Knob::Component::GetBuilderAttributes()
{
	...
	atts.push_back(std::pair<std::string, std::string>("flags", std::to_string(knob->GetFlags())));
	atts.push_back(std::pair<std::string, std::string>("msg", knob->GetMsg()));
...
void Knob::Component::SetBuilderAttributes(const std::vector<std::pair<std::string, std::string>>& attributes)
{
...
void Knob::Component::SetInfo(const std::vector<std::pair<std::string, std::string>>& attributes)
{
...
std::shared_ptr<Window::Backbone> Knob::Builder::Create(const Point& pos, const std::string& file_path)
{
	...
	ax::util::Flag flags = std::stoi(control.GetChildNodeValue("flags"));
	std::string msg = control.GetChildNodeValue("msg");

	ax::Xml::Node info_node = control.GetNode("info");

	ax::Knob::Info k_info;
	k_info.bgColorNormal = ax::Color::FromString(info_node.GetAttribute("bgColorNormal"));
	k_info.bgColorHover = ax::Color::FromString(info_node.GetAttribute("bgColorHover"));
	k_info.bgColorClicked = ax::Color::FromString(info_node.GetAttribute("bgColorClicked"));

	k_info.img_path = info_node.GetAttribute("img_path");
	k_info.selected_img_path = info_node.GetAttribute("selected_img_path");
	k_info.knob_size = ax::Size(info_node.GetAttribute("knob_size"));
...
void Knob::OnMouseLeftDragging(const Point& position)
{
	...
	_zeroToOneValue = util::Clamp<double>(_zeroToOneValue, 0.0, 1.0);
...
void Knob::SetValue(const double& value, bool callValueChangeEvent)
{
	...
	int cur_img = _nCurrentImg;
	_zeroToOneValue = util::Clamp<double>(value, 0.0, 1.0);
...
void Knob::OnPaint(GC gc)
{
	...
	if (_knobImg) {
		gc.DrawPartOfImageResize(_knobImg.get(), Point(_nCurrentImg * info.knob_size.w, 0), info.knob_size,
