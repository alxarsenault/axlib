 ...
 * ax::NumberScroll::Msg.
 */
NumberScroll::Msg::Msg(const double& value, ax::util::Control::Type type)
	: _value(value)
	, _type(type)
{
}
...
ax::util::Control::Type NumberScroll::Msg::GetType() const
{
	return _type;
}
...
//	txtInfo.font_color = ax::Color(0.0);
	...
	_value = ax::util::Clamp(_value, _range.left, _range.right);

	std::string v_str;
	switch (_type) {
	case ax::util::Control::Type::INTEGER:
...
//	btn_info.font_color = ax::Color(0.0, 0.0);
	...
	int size_y = rect.size.h - btn_pos.y + 1;
	btn_pos.y--;
...
{
	_value = ax::util::Clamp(value, _range.left, _range.right);
	...
	case ax::util::Control::Type::REAL:
...
{
	_txtbox->GetWindow()->dimension.SetSize(ax::Size(size.w - 20, size.h));
	_btn_up->GetWindow()->dimension.SetSize(ax::Size(20, size.h / 2 + 1));

	ax::Point btn_pos(_btn_up->GetWindow()->dimension.GetRect().GetNextPosDown(0));
	int size_y = size.h - btn_pos.y + 1;
	btn_pos.y--;
