 ...
 * ax::DropMenu::Flags.
 */
const ax::util::Flag ax::DropMenu::Flags::TEXT_ALIGN_CENTER = ax::util::FlagValue::FLAG_1;
const ax::util::Flag ax::DropMenu::Flags::TEXT_ALIGN_RIGHT = ax::util::FlagValue::FLAG_2;
...

{
	...
	if (_items.size()) {
		...
		if (total_height > rect.size.h) {
			int n_item = rect.size.h / info.item_height;
			...
			new_rect.size.h = n_item * info.item_height;
		}
		else {
			_over_flow = false;
			_over_flow_up = false;
			...
			new_rect.size.h = (int)_items.size() * info.item_height;
		}
	}
...
void ax::DropMenu::SetSelectedItem(const int& index)
{
	if (index >= (int)_items.size()) {
		ax::console::Error("ax::DropMenu::SetSelectedItem -> Index out of bound.");
		return;
	}
...
int ax::DropMenu::MousePosToIndex(const ax::Point& pos)
{
	...
	if (pos.y > win->dimension.GetRect().size.h) {
		return ITEM_NONE_SELECTED;
	}
	...
	if (_over_flow) {
		if (_over_flow_down && _over_flow_up) {
			...
			ax::console::Error("ax::DropMenu::MousePosToIndex -> Should never happen.");
		}
	}
...
void ax::DropMenu::OnMouseLeftDown(const ax::Point& pos)
{
	...
	if (_over_flow && (i < ITEM_NONE_SELECTED)) {
		if (_over_flow_down && _over_flow_up && (i == ITEM_SCROLL_UP)) {
			...
			ax::console::Error("ax::DropMenu::OnMouseLeftDown -> Should not happen (down)");
			_top_item_index++;
			win->Update();
			return;
		}
		...
		ax::console::Error("ax::DropMenu::OnMouseLeftDown -> Should never happen.");
		return;
	}
...
void ax::DropMenu::OnPaint(ax::GC gc)
{
	...
	if (_over_flow) {
		if (_over_flow_up && _over_flow_down) {
			index_offset = 1;
			// Up.
			ax::Rect up_rect(0, 0, rect.size.w, info->item_height);
			...
			ax::Rect down_rect(0, y, rect.size.w, info->item_height);
			...
			ax::console::Error("ax::DropMenu::OnPaint -> Should not happen (draw)");
		}
	...
	for (int i = 0; i < _n_shown_item; i++) {
		int index = i + _top_item_index;
		int y = (i + index_offset) * info->item_height;
		ax::Rect item_rect = ax::Rect(0, y, rect.size.w, info->item_height);
		gc.SetColor(draw_info[index].bg_color);
		gc.DrawRectangle(item_rect);
		gc.SetColor(draw_info[index].txt_color);
		draw_string(*_font.get(), draw_info[index].txt, item_rect);
		...
		for (auto& n : _separator_index) {
			if (index == n) {
				gc.SetColor(info->separation);
				gc.DrawLine(ax::Point(0, y), ax::Point(rect.size.w, y));
				break;
			}
		}
	}
